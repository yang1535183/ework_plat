...
<div id="fileuploadanddownloaddiv" style="width: 100%; height:100%; overflow:hidden;">
	<div id="fileuploadanddownload" style="width: 860px; height: 310px; overflow:hidden; margin-top: 40px; margin-left: 80px;">
...
<script type="text/javascript">
		...
		$('#fileuploadanddownloadtable').datagrid({
			...
			columns: [[
				...
				{field:'CZ',title:'操作',width:120,align:'center',formatter:function(value, row, index) {
					var _a = '<a href="javascript:void(0);" onclick="uploadFj(\''+row.id+'\',\''+row.dm_code+'\',\''+row.dm_name+'\',\''+row.gcid+'\');"><span class="icon-redo">&nbsp;</span>[上传]</a>'; 
	...
	function uploadFj(id, dmcode, dm_name, _gcid) {
		if('null' != _gcid) {
			$.messager.confirm('消息', '系统检测'+dm_name+'，已上传，是否覆盖已上传数据！！！', function(r){
				if (r){
					fileUpload(id, dmcode, dm_name);
				}
			});
		}
		else {
	...
	function fileUpload(_id, dmcode, dm_name) {
		...
		_html +='					<input type="button" id="btnImpData" class="button_bg" value="上传" />';
		_html +='				</td>';
		_html +='			</tr>';
		_html +='		</table>';
		_html +='		</form>';
...
//		$('#fildImpData').dialog('close').remove();
	...
	//	$("#spanImpData").empty().append('<input type="button" id="btnImpData" class="button_bg" value="上传" />');
		var _form = '#ppgxaddandmcheckform';
		if('${sjzt}' == 0) {
			_form = '#ppgxaddandmmodelform';
		}
		if('null' != _id) {
			var args = {pzmc:'UPLOADH5', id: _id, xmlx:dmcode, gcid: $(_form+' #id').val()};
		}
		else {
			var args = {pzmc:'UPLOADH5', xmlx:dmcode, gcid: $(_form+' #id').val()};
		}
		...
		$("#btnImpData").each(function() {
			new AjaxUpload(this, {
				...
				onComplete: function(file, response) {
					...
					result = eval('('+result+')');
					if (result.success) {
						$(_form+' #id').attr("value", result.gcid);
	...
	function fileQuery() {
		...
		var queryParams_ = $('#fileuploadanddownloadtable').datagrid('options').queryParams;
		queryParams_.gcid = $(_form+' #id').val();
		$('#fileuploadanddownloadtable').datagrid('options').queryParams = queryParams_;
		$('#fileuploadanddownloadtable').datagrid('clearSelections').datagrid('reload');
	}
</script>